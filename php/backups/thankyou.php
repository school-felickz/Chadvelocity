...
$query = "CREATE TABLE IF NOT EXISTS $table_name ";
$query = $query . "(Oid INT UNSIGNED NOT NULL AUTO_INCREMENT PRIMARY KEY, ";
	...
	foreach($cart->getContents() as $item) //Use of cart getContents loop was adapted from webforce.co.nz/cart
	 {
		...
		if(ctr == 0)
		{
			$query = "INSERT INTO $table_name(ProductName, Quantity, Name, Address) ";
			$query = $query . "VALUES('".$product['name']."', '".$item['qty']."', '";
		...
		{
			$query = "INSERT INTO $table_name(ProductName, Quantity, Name, Address) ";
			$query = $query . "VALUES('DAAF', '0000', 'DRTAN', 'cheese')";
			if (!mysql_query($query, $connect)) die('insert into failed, ' . mysql_error());
		}
...
print '<th colspan=6>Purchases Database';
while ($row = mysql_fetch_row($results_id))
{
//$date=date("m-d-Y H:i:sa",$row[1]); 
        ...
        print "<td>$row[2]</td>";         //Name of the product
        print "<td>$row[3]</td>";         //Name of the product
        print "<td>$row[4]</td>";         //Name of the product
        print "<td>$row[5]</td>";         //Name of the product
        print '</tr>';
